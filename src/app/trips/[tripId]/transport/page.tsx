...
import { TransportClient } from "./_client";
...
export default async function TransportPage({ params }: Props) {
  ...
  const canManage = membership.role === "organizer" || membership.role === "activity_manager";

  const { data: transports } = await supabase
    .from("transports")
    .select(`
      *,
      assignments:transport_assignments(
        *,
        ...
        <nav className="flex gap-2 mb-8 flex-wrap">
          {NAV(tripId).map(({ label, href }) => (
            <LinkButton
              key={href}
              href={href}
              variant={href.endsWith("/transport") ? "default" : "outline"}
              size="sm"
            >
              {label}
            </LinkButton>
